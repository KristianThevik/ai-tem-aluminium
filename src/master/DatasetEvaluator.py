    ...
    def statistics(self):
        ...
        for id, image in enumerate(self.image_paths):
            ...
            for i in range(len(prediction)):
                ...
                mean_val.append(prediction[i])
                confidence_scores.append(scores[i])

            mean_per_image.append(np.mean(np.array(mean_val)))
            density_per_image.append(len(mean_val)/(self.size*self.nm_per_px[id])**2)
        ...
        # Multiply self.size with 2 above since images are originally 2048x2048 and the original nm_per_px is used, but not yolo
        # Problem above, need case for self.size * 2 (not yolo) and and (self.size *1)(yolo)
        if self.type == 'cross':
        ...
        stats_dict = {
            ...
            'Mean per image' : list(np.round(mean_per_image, 1)) + [np.nan]* (len(len_cross_vals)-len(np.unique(np.array(image_ids)))), 
            ...
            'Number Density [nm^-2]': list(np.round(density_per_image, 7)) + [np.nan]* (len(len_cross_vals)-len(np.unique(np.array(image_ids))))  
        }

        df = pd.DataFrame(stats_dict)

        df.to_csv(os.path.join(os.path.dirname(self.dataset_dir), f'statistics_{file}.csv'))
        ...
        file_path = "var4_DAA_cross.csv" if self.type == 'cross' else "var4_DAA_length.csv" # DF functionailty needed
        ...
        x_max = max(max(hist_vals), max(hist_numbers))
        x_vals = np.linspace(0, x_max + 2, 1000)  # Add small buffer if needed

        # KDE and normalization
        kde_yolo = gaussian_kde(hist_vals)
        pdf_yolo = kde_yolo(x_vals)
        pdf_yolo = pdf_yolo / np.trapz(pdf_yolo, x_vals)  # Normalize to area=1

        kde_manual = gaussian_kde(hist_numbers)
        pdf_manual = kde_manual(x_vals)
        pdf_manual = pdf_manual / np.trapz(pdf_manual, x_vals)  # Normalize to area=1


        x_lim = 120

        if self.type == 'cross' or self.type == 'dark field':
            #x_lim = 30
            x_lim = np.max(number_list) + 10
        else:
            x_lim = np.max(number_list) + 10
        ...
        ax.set_xlabel(f"{dark_field_prefix} Cross Section [nm^2]" if (self.type == 'cross' or self.type == 'dark field') else "Length [nm]")  # DF rephrase Adjust based on your measurement
        ax.set_xlim(0, max(hist_vals))
        ax.set_ylabel("Number Frequency")
        ax.set_title(f"Distribution of {dark_field_prefix} Cross-Sectional Areas" if (self.type == 'cross' or self.type == 'dark field') else "Distribution of Lengths") #DF rephrase
        ...
        plt.plot(x_vals, pdf_yolo, color="blue", label="YOLO")
        plt.plot(x_vals, pdf_manual, color="red", label="Manual")
        
        
        plt.xlabel(f"{dark_field_prefix} Precipitate Cross Section [nm^2]" if (self.type == 'cross' or self.type == 'dark field') else "Precipitate Length [nm]", fontsize=22)
        plt.ylabel("Normalized Distribution" + (" [1/nm^2]" if (self.type == 'cross' or self.type == 'dark field') else " [1/nm]"), fontsize=22)
        plt.title("Var4_DAA_5h_185C: Manual vs Automatic Distribution of " + (f"{dark_field_prefix} Cross Sections" if (self.type == 'cross' or self.type == 'dark field') else "Lengths"), fontsize=26)
...
class YOLOEvaluator(Evaluator):
    ...
    def predict(self, img, nm_px_ratio):
        ...
        if self.type == 'dark field':   # Contrast enhancement for dark field images
        ...
        result = self.model.predict(img, conf=self.threshold, max_det=1900)[0]
        ...
        if self.type == 'cross':
            ...
            for i, mask in enumerate(masks):
                binary_mask = mask > 0  # Convert to binary

                # Erode the mask because cross sections borders can be somewhat thick
                kernel = np.ones((3, 3), np.uint8)
                eroded_mask = cv2.erode(binary_mask.cpu().numpy().astype(np.uint8), kernel, iterations=4)

                # Remove particles touching image border
                cleaned_mask = clear_border(eroded_mask)

                # Area calculation
                pixel_count = np.sum(cleaned_mask)
                area = pixel_count * ((nm_px_ratio) ** 2)
                # Make sure accepted cross sections have a minimum area of 1 nm^2.
                if area >= 1:
                    self.area.append(area)
                    overlay[cleaned_mask > 0] = [255,255,0]
                    ...
                    if len(x_coords) > 0 and len(y_coords) > 0:
                        ...
                        overlay[centroid_y - 16:centroid_y + 16, centroid_x - 16:centroid_x + 16] = [255, 0, 0]
            ...
            # If dark field is instance seg model, it can be equal to 'cross' case
            # Change this back to dots.
            ...
            for i, mask in enumerate(masks):
                ...
                kernel = np.ones((3, 3), np.uint8)
                binary_mask_eroded = cv2.erode(binary_mask_np.astype(np.uint8), kernel, iterations=3)
                ...
                if len(points) >= 5:  # Ensure sufficient points for minAreaRect
                    ...
                    (center), (width, height), angle = rect
                    length = np.max([width, height]) * nm_px_ratio
                    self.lengths.append(length)
